...
export default function CameraFeed({
  ...
  const [hasError, setHasError] = useState(false); // State để bắt lỗi stream
  ...
  // Cập nhật nguồn ảnh khi props thay đổi
  useEffect(() => {
    setHasError(false); // Reset lỗi khi đổi camera
    if (useWebcam && status !== "offline") {
      setImgSrc(API_URLS.STREAM(camId));
    } else {
      setImgSrc(src);
    }
  }, [useWebcam, status, camId, src]);
